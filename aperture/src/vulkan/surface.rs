...
impl Surface {
    ...
    pub fn resolution(&self) -> Option<vk::Extent2D> {
        self.properties
            .as_ref()
            .map(|properties| properties.resolution)
    }
    ...
    pub fn desired_image_count(&self) -> Option<u32> {
        self.properties
            .as_ref()
            .map(|properties| properties.desired_image_count)
    }
    ...
    pub fn pre_transform(&self) -> Option<vk::SurfaceTransformFlagsKHR> {
        self.properties
            .as_ref()
            .map(|properties| properties.pre_transform)
    }
}
...
impl Drop for Surface {
    fn drop(&mut self) {
        unsafe {
            self.ash_handle.destroy_surface(self.vk_handle, None);
        }
    }
}
